...
def _get_historical_lags(state, district, crop, cropyear):
    ...
    lag_cols = [
        "yieldcalc", "production", "area",
        "Rainfall_sum", "Tavg_mean",
        "Tmax_mean", "Tmin_mean",
        "ET0_sum", "GDD_sum"
    ]

    # Default safe structure
    lags = {f"{c}_lag1": 0.0 for c in lag_cols}

    if df.empty:
        return lags

    sn = _norm_text(state)
    dn = _norm_text(district)
    ...
    subset = df[
        ...
        (df["cropyear"] < cropyear)
    ]

    if subset.empty:
        return lags

    # Use most recent available historical year
    latest_year = subset["cropyear"].max()
    hist = subset[subset["cropyear"] == latest_year]

    for col in lag_cols:
        if col in hist.columns:
            val = pd.to_numeric(hist[col], errors="coerce").dropna()
            if not val.empty:
                lags[f"{col}_lag1"] = float(val.iloc[0])
...
# =============================================================================
# =============================================================================
# Core Prediction - Ridge Forward Chain Stacking (FULLY FIXED)
# =============================================================================
def _predict_core(state: str, district: str, crop: str,
                  land_area: float, sowing_date: str,
    ...
    for k, v in weather_defaults.items():
        weather_cumulative.setdefault(k, v)

    # -------------------------------------------------------------------------
    # 4. Historical Lags (latest available year < cropyear)
    # -------------------------------------------------------------------------
    lags = _get_historical_lags(state, district, crop, cropyear)

    # Ensure lag columns always exist
    lag_columns = [
        "yieldcalc_lag1", "production_lag1", "area_lag1",
        "Rainfall_sum_lag1", "Tavg_mean_lag1",
        "Tmax_mean_lag1", "Tmin_mean_lag1",
        "ET0_sum_lag1", "GDD_sum_lag1"
    ]
    for col in lag_columns:
        lags.setdefault(col, 0.0)

    # -------------------------------------------------------------------------
    # 5. Delta Features (time-consistent)
    # -------------------------------------------------------------------------
    deltas = {}
    ...
    for col in weather_cols:
        current_val = float(weather_cumulative.get(col, 0.0))
        lag_val = float(lags.get(f"{col}_lag1", 0.0))
        deltas[f"{col}_delta1"] = current_val - lag_val

    # Structural features (no artificial delta drift)
    for col in ["yieldcalc", "production", "area"]:
        deltas[f"{col}_delta1"] = 0.0

    # Ensure all delta columns exist
    delta_columns = [
        "yieldcalc_delta1", "production_delta1", "area_delta1",
        "Rainfall_sum_delta1", "Tavg_mean_delta1",
        "Tmax_mean_delta1", "Tmin_mean_delta1",
        "ET0_sum_delta1", "GDD_sum_delta1"
    ]
    for col in delta_columns:
        deltas.setdefault(col, 0.0)

    # -------------------------------------------------------------------------
    # 6. Build Model Input (ALL REQUIRED COLUMNS)
    # -------------------------------------------------------------------------
    input_dict = {
        "statename": state,
    ...
    # -------------------------------------------------------------------------
    # 7. Forward Chain Fold Predictions
    # -------------------------------------------------------------------------
    base_preds = []
    ...
    # -------------------------------------------------------------------------
    insights = _analyze_confidence_and_risks(
        weather_cumulative,
    ...
    # -------------------------------------------------------------------------
    # 9. Final Return Structure (UI expects this exact format)
    # -------------------------------------------------------------------------
    return {
        "state": state,
        "district": district,
        ...
        "fold_agreement": (
            "High" if fold_std < 0.3
            else "Medium" if fold_std < 0.6
            else "Low"
        )
    }
